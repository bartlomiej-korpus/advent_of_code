...
defmodule RemoteControl do
  ...
  def run do
    ...
    map =
      scan(state, droid_location, %{{0, 0} => 1})

    {oxygen_location, _} = Enum.find(map, fn {_, type} -> type == 2 end)

    spread_oxygen(map, [oxygen_location])
  end

  def spread_oxygen(map, minute \\ 0, to_spread)
  def spread_oxygen(_, minute, []), do: minute - 1
  def spread_oxygen(map, minute, to_spread) do
    spreading_to = for oxygen_point <- to_spread, {direction, _} <- @directions do
      elem(walk({oxygen_point, direction}), 0)
    end
    |> Enum.filter(fn coordinates ->
      Map.get(map, coordinates) == 1
    end)

    map = Enum.reduce(spreading_to, map, fn coordinates, map ->
      Map.put(map, coordinates, 2)
    end)

    spread_oxygen(map, minute + 1, spreading_to)
  end

  def scan(state, droid_location, map, step \\ 1) do
    results =
      @directions
    ...
    to_visit = Enum.filter(results, fn {_, result, _, _} -> result != 0 end)

    map = Enum.reduce(results, map, fn {_, _, _, map2}, map1 -> Map.merge(map1, map2) end)

    case length(to_visit) do
      0 ->
        map

      _ ->
        to_visit
        |> Enum.map(fn {new_coordinates, _, state, _} ->
          scan(state, new_coordinates, map, step + 1)
        end)
