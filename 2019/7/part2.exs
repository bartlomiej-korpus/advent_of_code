defmodule IntCodeIO do
  ...
  def handle_call(:get_all, _, {all, fun}), do: {:reply, all, {all, fun}}
...
defmodule AmplificationCircuit do
  def create(seq) do
    ...
    IntCodeIO.set_on_output(amp_e_io, fn output ->
      IntCodeIO.add_next(amp_a_io, output)
    ...
    {[thruster_io, amp_a_io, amp_b_io, amp_c_io, amp_d_io, amp_e_io], tasks}
  end
end
...
Permutations.generate([5, 6, 7, 8, 9])
|> Enum.map(fn seq ->
  seq_str = seq |> Enum.join()

  circuit = AmplificationCircuit.create(seq)

  {[thruster_io | _], intcode_tasks} = circuit

  Enum.map(intcode_tasks, &Task.await/1)

  last_thruster_output = IntCodeIO.get_all(thruster_io) |> Enum.at(-1)

  Enum.map(elem(circuit, 0), fn pid ->
    GenServer.stop(pid)
  end)

  {seq_str, last_thruster_output}
end)
